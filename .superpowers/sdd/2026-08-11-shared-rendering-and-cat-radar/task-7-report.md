...
## Fix round 4/5 — backend-bound resources and strict frame/result separation

Round 4 started clean at exact base 07d5f992b1eb56c4ffa4c9ff9ce4cc8abb45983e on codex/franz-lola-shared-rendering. receiving-code-review, systematic-debugging, TDD/writing-good-tests, and verification-before-completion governed the work. Both findings reproduced before production edits.

The resource root cause was applicability-driven validation without a backend discriminator. The central validator now binds exact schemas to backend: Canvas2D requires exactly not-applicable plus canvas2d-cpu-compositor, finite backingStoreResizes, and no GPU fields; WebGL2/WebGPU require exactly applicable, all nine finite GPU fields, and no Canvas field. Unknown backends and unknown/missing resourceMetrics keys fail closed. Its normalized return includes every already-validated counter. Game, Studio, benchmark, and Renderer gates consume that one validator rather than maintaining divergent subsets.

PresentationFrame is again strictly nine root fields for create/is/serialize. Public render() remains a frozen 12-key compatibility result with current referential playerScreen, entities, and characterEntities, but isPresentationFrame(result) is false and direct serialization throws. The separate explicit presentationFrameFromRenderResult() validates the exact wrapper, descriptor shape, deep-frozen canonical frame, and alias identity before returning a strict nine-key frame. Game debug capture and Studio diagnostic capture explicitly unwrap at that boundary; no alias is silently stripped by the PresentationFrame API.

### RED -> GREEN and gates

- Initial two-file RED: Exit 1, 0/4. It proved Canvas+GPU schema acceptance, missing normalized skip counters, 12-key isPresentationFrame=true, and absent explicit unwrap API. Resource GREEN was 28/28; strict result/frame GREEN 2/2. Studio consumer RED was 0/1, then GREEN through explicit unwrap. Combined affected integration passed 64/64.
- npm test: Exit 0 in 50 s, 588/588 (root40, Game157, Publisher21, Studio142, content49, core36, Renderer106, coordinator30, testkit7).
- npm run build: Exit 0 in 10.2 s; Renderer 39 modules/215.53 kB (58.16 gzip), Game205/424.66 (121.96), Studio206/455.32 (126.44).
- Benchmark assert: Exit 0 in 111 s, generated 2026-08-13T10:02:14.060Z. Weak-mobile auto gameplay/cutscene render p95 16.7/18.3, frame p95 33.4/33.4, long 17.32/18.99% against unchanged 36/52/50; invalid/failure arrays empty. Canvas had only N/A/backing-store data; GPU had all nine finite fields. No DevTools metric was invented.
- Real canaries: Renderer port63119, 3 scenarios/6 artifacts/pacer121/121/121, structured WebGPU skip; Game run run-2026-08-13T10-02-57-927Z passed WebGL2; Studio Canvas run run-2026-08-13T10-02-54-314Z-25032 port49643 passed with forced=false portClosed=true.

| Gate | Renderer | Game | Studio E2E / Visual / Rendering |
|---|---|---|---|
| Browser pass 1, Exit0, 910.4s | 2026-08-13T10-03-46-010Z-31268, port55224, 3/6, pacer121/121/121, WebGPU structured skip | run-2026-08-13T10-04-05-183Z, port50612, 14/14, 90 PNG + 14 WebM, 29,600,599 bytes, min21.12s | E2E run-2026-08-13T10-09-56-219Z-28860 port53566 40/40; Visual run-2026-08-13T10-11-56-626Z-27624 port60572 9/9; Rendering run-2026-08-13T10-12-55-556Z-7628 port52550 15/15 PNG/WebM, 17,141,527 bytes, min20.64s |
| Browser pass 2, Exit0, 909.1s | 2026-08-13T10-19-14-387Z-2712, port58684, same complete status | run-2026-08-13T10-19-33-631Z, port61674, 14/14, 90 PNG + 14 WebM, 29,475,723 bytes, min21.04s | E2E run-2026-08-13T10-25-26-042Z-15164 port61071 40/40; Visual run-2026-08-13T10-27-23-510Z-33004 port54003 9/9; Rendering run-2026-08-13T10-28-21-956Z-35092 port54240 15/15, 17,107,891 bytes, min20.68s |

Both passes retained all five-second, health, compositor PNG, video, mobile/landscape/60/120/175/reduced, WebGL2/Canvas2D and WebGPU rules. Renderer/Game recorded the real requestAdapter() returned null; Studio native WebGPU passed. All wrappers reported code=0 forced=false portClosed=true.

npm ci --ignore-scripts exited 0 in 4.8 s (87 installed, 96 audited, zero vulnerabilities). Final npm run verify exited 0 in 1091.3 s and reran all 588 tests, builds, benchmark, and browser: Renderer 2026-08-13T10-37-42-667Z-31304 port65467; Game run-2026-08-13T10-38-01-899Z port50899, 14/14, 90 PNG + 14 WebM, 29,644,160 bytes, min21.16s; Studio E2E run-2026-08-13T10-43-55-554Z-9632 port49449 40/40, Visual run-2026-08-13T10-45-57-671Z-10220 port57884 9/9, Rendering run-2026-08-13T10-46-57-435Z-33340 port53297 15/15, 17,314,763 bytes, min20.72s, native WebGPU passed. Cleanup remained non-forced and closed.

### Scope, cleanup, ACL, and concerns

Scope is limited to shared resource/frame-result boundaries, their Game/Studio/Renderer/benchmark consumers, tests, and this report. Root/CI composition required no change and its contracts passed. No budgets, tolerance, artifacts, WebGPU rule, or product feature were altered. Final audit found zero listeners on all recorded ports and zero relevant Node gate processes. Generated .last-run.json was removed only after exact SHA/marker guards. Existing-file apply_patch attempts hit the known deny-read ACL; exact path/SHA/single-anchor fallbacks were diff-/syntax-checked immediately, while new files used apply_patch. No approval was denied.

The Task5 coalesced pointer-reason disposition remains unchanged: first pending reason is provenance and latest pointer-up state is presented once. Environmental concerns remain only honest WebGPU context variance and the Windows ACL preventing a fresh manual view_image; decoded compositor PNG and mandatory WebM gates passed throughout. No known product, schema, compatibility, budget, artifact, process, or port failure remains after Round4.